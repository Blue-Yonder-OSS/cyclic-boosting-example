...
def calculate_factors(est, X):
    ...
    df_factors['yhat_mean'] = df_factors['yhat_mean'] / est[-1].global_scale_
    df_factors['y'] = df_factors['y'] / est[-1].global_scale_
...
def prepare_data(df):
    ...
    df['event_type_1'].fillna("NAN", inplace=True)
    enc = OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=np.nan)
    df[['store_id', 'item_id', 'event_type_1']] = enc.fit_transform(df[['store_id', 'item_id', 'event_type_1']])
...
def mean_fit(X, y, split_date):
    ...
    plot_CB('analysis_CB_mean_iterlast', [ml_est_mean[-1].observers[-1]], ml_est_mean[-2])
...
def width_fit(X, split_date):
    ...
    plot_CB('analysis_CB_width_iterlast', [ml_est_width[-1].observers[-1]], ml_est_width[-2])
